var p = {
    close: {
        x: 1855, y: 62,
        ifColor: { r: 230, g: 205, b: 188, a: 255 }
    },
    noWindows: {
        menuSwitcher: {x: 89, y: 75},
        craft: {x: 636, y: 82},
        quest: {x: 1400, y: 85},
        directions: {
            top: {x: 0, y: 0},
            bottom: {x: 0, y: 0},
            left: {x: 0, y: 0},
            right: {x: 0, y: 0},
            topRight: {x: 340, y: 797}
        },
        dungeon: {x: 1374, y: 210},
        event: {x: 1245, y: 210},
        autoBattling: {x: 1605, y: 770},
        exchange: {x: 1608, y: 594},
        arena: {x: 1116, y: 218},
        vip: {x: 432, y: 92},
        inventory: {
            x: 1415, y: 1013,
            ifColor: { r: 91, g: 146, b: 125, a: 255 }
        },
        system: {
            x: 1373, y: 84,
            meditation: {x: 1178, y: 248}
        },
        map: {x: 1745, y: 203},
        questLog: {
            dailyQuest: {
                x: 163, y: 555,
                ifColor: { r: 1, g: 218, b: 3, a: 255 }
            },
            switcher: {
                x: 377, y: 559,
                daily: {x: 464, y: 557},
                target: {x: 579, y: 561}
            }
        }
    },
    questWindow: {
        daily: {
            x: 116, y: 369,
            normalExp: {x: 1328, y: 984},
            x2Exp: {x: 1695, y: 983},
            upgrade: {x: 1721, y: 845},
            completeAll: {x: 625, y: 618},
            go: {
                x: 1683, y: 985,
                ifColor: { r: 53, g: 92, b: 0, a: 255 }
            }
        },
        target: {
            x: 114, y: 503,
            go: {x: 1518, y: 980},
            complete: {x: 1518, y: 980}
        }
    },
    battleWinDialog: {
        close: {x: 1470, y: 89},
        normalExp: {x: 536, y: 756},
        x2Exp: {x: 821, y: 753},
        x3Exp: {x: 1091, y: 752},
        accept: {x: 961, y: 958}
    },
    insideDungeon: {
        exit: {x: 1528, y: 50}
    },
    mapWindow: {
        worldMap: {
            x: 164, y: 186,
            lorencia: {x: 839, y: 568}
        },
        areaMap: {
            x: 169, y: 301,
            npc: {x: 1632, y: 176},
            monster: {x: 1625, y: 261},
            lorencia: {
                npc: {
                    littleGirlAmy: {x: 1570, y: 786}
                }
            },
            colosseum: {
                monster: [
                    {},
                    {},
                    {},
                    {},
                    {x: 1574, y: 600}
                ]
            },
            kalima: {
                monster: {
                    kundunsIllusion: {
                        x: 1556, y: 784
                    }
                }
            }
        },
        close: {
            x: 1850, y: 60,
            ifColor: { r: 230, g: 205, b: 188, a: 255 }
        }
    },
    dialogWindow: {
        close: {x: 1701, y: 369}
    },
    meditationWindow: {
        zen: {x: 694, y: 633},
        close: {x: 1494, y: 124}
    },
    characterWindow: {
        inventory: {
            x: 1415, y: 1013,
            disassemble: {
                x: 1199, y: 987,
                disassembleDialog: {
                    disassemble: {x: 439, y: 981}
                }
            }
        }
    },
    vipWindow: {
        features: {
            x: 703, y: 196,
            potionShop: {x: 1483, y: 579}
        }
    },
    arenaRewardsWindow: {
        claim: {x: 289, y: 914}
    },
    potionShopWindow: {
        largeHpPotion: {x: 344, y: 418},
        smallMpPotion: {x: 852, y: 415},
        countDialog: {
            max: {x: 1212, y: 702},
            buy: {x: 1422, y: 776},
            close: {x: 1337, y: 188}
        } 
    },
    arenaWindow: {
        ranking: {x: 575, y: 230},
        reward: {x: 259, y: 230},
        close: {x: 1776, y: 77}
    },
    rankingWindow: {
        zen: {x: 164, y: 429},
        arena: {x: 259, y: 230},
        positions: [
            {x: 802, y: 263},
            {x: 794, y: 342},
            {x: 792, y: 422},
            {x: 809, y: 500},
            {x: 822, y: 581}
        ],
        cheer: {x: 1555, y: 956},
        announcementDialog: {
            confirm: {
                x: 1149, y: 707,
                ifColor: { r: 92, g: 142, b: 1, a: 255 }
            }
        }
    },
    exchangeWindow: {
        close: {x: 1713, y: 93},
        exchange: {x: 1478, y: 891},
        diamonds: {x: 458, y: 400},
        boundDiamonds: {x: 957, y: 400},
        zen: {x: 1458, y: 400}
    },
    craftWindow: {
        accessTicket: {
            x: 164, y: 183,
            devilSquareTicket: {x: 1558, y: 244},
            bloodCastleCloak: {x: 1531, y: 428},
            craft: {x: 781, y: 983}
        },
        fruit: {
            x: 164, y: 312,
            fruit: {x: 1558, y: 244},
            craft: {x: 781, y: 983}
        }
    },
    storyWindow: {
        enableSweep: {x: 213, y: 1008},
        selectNormal: {x: 877, y: 534},
        selectElite: {x: 1270, y: 534},
        selectHell: {x: 1670, y: 534},
        sweep: {x: 1297, y: 1003},
        enter: {x: 1297, y: 1003}
    },
    dailyEventWindow: {
        enter: {x: 1617, y: 1003}
    },
    eventWindow: {
        dailyEvent: {
            x: 164, y: 183,
            screens: [
                {
                    x: 1000, y: 983,
                    list: [
                        {title: 'Devil Square', x: 490, y: 540},
                        {title: 'Blood Castle', x: 910, y: 540},
                        {title: 'Angel Temple', x: 1290, y: 540},
                        {title: 'Chaos Castle', x: 1660, y: 540}
                    ]
                },
                {
                    x: 1086, y: 983,
                    list: [
                        {title: 'Illusion Temple', x: 490, y: 540}
                    ]
                }
            ]
        },
        worldBoss: {
            x: 0, y: 0
        },
        goldenForces: {
            x: 0, y: 0
        },
        vipOnly: {
            x: 0, y: 0
        },
        colosseum: {
            x: 170, y: 670,
            enter: {x: 1695, y: 969}
        },
        crystalMine: {
            x: 0, y: 0
        }
    },
    dailyDungeonWindow: {
        enter: {x: 1617, y: 1001}
    },
    announcementDialog: {
        confirm: {
            x: 809, y: 668
        },
        cancel: {x: 1105, y: 665},
        close: {x: 1248, y: 376},
    },
    dungeonWindow: {
        storyDungeon: {
            x: 164, y: 183,
            screens: [
                {
                    x: 919, y: 983,
                    list: [
                        {title: 'Death King Chapel', x: 490, y: 540},
                        {title: 'Secret Forest', x: 910, y: 540},
                        {title: 'Ice Queen Palance', x: 1290, y: 540},
                        {title: 'Gorgon Castle', x: 1660, y: 540}
                    ]
                },
                {
                    x: 1000, y: 983,
                    list: [
                        {title: 'Balrog Castle', x: 490, y: 540},
                        {title: 'Hydra Den', x: 910, y: 540},
                        {title: 'Flame Cave', x: 1290, y: 540},
                        {title: 'Phoenix Chapel', x: 1660, y: 540}
                    ]
                }, 
                {
                    x: 1085, y: 983,
                    list: [
                        {title: 'Tranquil Forest', x: 490, y: 540},
                        {title: 'Ancient Coast', x: 910, y: 540},
                        {title: 'Energy Storage', x: 1290, y: 540}
                    ]
                }
            ]
        },
        dailyDungeon: {
            x: 162, y: 308,
            expDungeon: {x: 522, y: 459},
            zenDungeon: {x: 907, y: 485}
        },
        partyDungeon: {
            x: 0, y: 0,
            kalima: {x: 0, y: 0},
            corpsOnslaught: {x: 0, y: 0},
            labirynthOfDimensions: {x: 0, y: 0}
        },
        endlessTower: {
            x: 159, y: 543,
            sweep: {x: 1434, y: 973},
            accept: {x: 1718, y: 972},
            challenge: {x: 1718, y: 972}
        }
    }
};

module.exports = p;